...
class Sender {
  ...
  send (data, options, cb) {
    ...
    if (perMessageDeflate) {
      ...
      /**
       * we are not able to guess whether buffer is flushed to kernel, 
       * so we simply return false in this case
       **/
      return false; 
    } else {
      return this.sendFrame(Sender.frame(data, {
        fin: options.fin,
        rsv1: false,
        opcode,
        mask: options.mask,
  ...
  sendFrame (list, cb) {
    if (list.length === 2) {
      ...
      return this._socket.write(list[1], cb);
    } else {
      return this._socket.write(list[0], cb);
    }
  }
}
