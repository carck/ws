...
class WebSocket extends EventEmitter {
  ...
  setSocket (socket, head) {
    ...
    // export drain event, so that we can support pipe
    this._ultron.on('drain', () => {
      this.emit('drain');
    });
  ...
  send (data, options, cb) {
    ...
    return this._sender.send(data || constants.EMPTY_BUFFER, opts, cb);
  }
...
function initAsClient (address, protocols, options) {
  ...
  this._req.on('upgrade', (res, socket, head) => {
    ...
    this._req.removeAllListeners('timeout');
    this._req = null;
